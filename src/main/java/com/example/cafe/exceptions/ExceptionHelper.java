...
public class ExceptionHelper {
    ...
    public ResponseEntity<?> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException ex, RedirectAttributes redirectAttributes) {
        redirectAttributes.addFlashAttribute("errorMessage", "Maximum upload size exceeded. Please upload a smaller file.");
        return new ResponseEntity<>(ApiResult.errorResponse(MessageConstants.FILE_SIZE_EXCEEDED, 400), HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<?> handleException(IOException ex) {
        ex.printStackTrace();
        return new ResponseEntity<>(
                ApiResult.errorResponse("COULD NOT SAVE FILES", 500),
                HttpStatus.CONFLICT);
    }
